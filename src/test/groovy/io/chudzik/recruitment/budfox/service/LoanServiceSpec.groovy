...
class LoanServiceSpec extends BaseUnitSpec {
    ...
    @Subject LoanService sut = new LoanServiceImpl(clientRepoMock, loanRepoMock, conditionsServiceMock)
    ...
    def "should persist issued loan to db"() {
        ...
        when:
            sut.issueALoan(loanApplication())
        then:
            1 * loanRepoMock.save(unsavedLoan())
    }
    ...
    def "should persist extended loans to db"() {
        ...
        then:
            ...
            1 * loanRepoMock.save(loanAfterFirstExtension())
    }
